...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Meta-EBM on DeepSeek PEFT LoRA adapter checkpoints")
    ...
    parser.add_argument(
        "--task_order",
        type=str,
        default="cola,mnli_m,mnli_mm,mrpc,qnli,qqp,rte,sst2,stsb,wnli",
    )
    ...
    parser.add_argument("--exp_name", type=str, default="meta_ebm_deepseek")
    ...
    parser.add_argument("--eval_init_mode", type=str, default="prototype", choices=["prototype", "random"])
    parser.add_argument("--eval_random_init_std", type=float, default=1.0)
    parser.add_argument("--eval_random_init_seed", type=int, default=-1)
...
def build_auto_eval_args(
    ...
    return argparse.Namespace(
        ...
        lora_project_dir=args.eval_lora_project_dir or os.path.join(project_root, "models", "LoRA_deepseek"),
        lora_config_dir=args.eval_lora_config_dir or os.path.join(project_root, "models", "config", "LoRA_deepseek"),
        ...
        random_init_std=args.eval_random_init_std,
        random_init_seed=args.eval_random_init_seed,
...
def train(args: argparse.Namespace) -> str:
    ...
    checkpoint_root = os.path.abspath(
        os.path.expanduser(
            ...
            or os.path.join(project_root, "CMA_ES", "checkpoints")
        )
    )
    ...
    if resume_payload is not None and resume_payload.get("task_embeddings"):
        ...
        task_embeddings, task_condition_metadata = build_task_condition_embeddings(
            ...
            default_lora_config_name="LoRA_deepseek",
        )
    ...
    for task_idx, task_name in enumerate(tasks[start_task_idx:], start=start_task_idx):
        ...
        if args.eval_after_each_task and (not args.dry_run or args.auto_eval_on_dry_run):
            ...
            eval_tasks_for_stage = select_after_task_eval_tasks(args, tasks, task_idx)
            if eval_tasks_for_stage:
                print(
                    f"[auto-eval] task {task_idx + 1}/{len(tasks)} finished; "
                    f"evaluating tasks={eval_tasks_for_stage}"
                )
                model.to("cpu")
                if torch.cuda.is_available():
    ...
    if args.auto_eval_after_train and args.eval_after_each_task:
        ...
        print("[auto-eval] training finished, running Meta-EBM refinement + GLUE evaluation")
        model.to("cpu")
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        from eval_meta_ebm import run_evaluation
