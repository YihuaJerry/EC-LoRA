...
from output_guard import guarded_makedirs
...
NON_LORA_TRAINABLES_FILENAME = "non_lora_trainables.bin"
...
def _load_safetensors(path: str) -> Dict[str, torch.Tensor]:
    try:
        ...
        raise ModuleNotFoundError(
            ...
            "Install it in the LoRA_Qwen conda env or use checkpoints with adapter_model.bin."
        ) from exc
...
def _read_tensor_state(path: str) -> Dict[str, torch.Tensor]:
    ...
    if isinstance(obj, dict):
        for key in ("state_dict", "model_state_dict", "trainable_state_dict"):
            ...
            if isinstance(nested, dict) and all(isinstance(v, torch.Tensor) for v in nested.values()):
                return nested
    raise ValueError(f"Unable to read a tensor state dict from {path}")


def load_adapter_model_state(checkpoint_dir: str) -> Dict[str, torch.Tensor]:
    return _read_tensor_state(find_adapter_weight_file(checkpoint_dir))


def find_non_lora_trainables_file(checkpoint_dir: str) -> str:
    return os.path.join(checkpoint_dir, NON_LORA_TRAINABLES_FILENAME)


def load_non_lora_trainables_state(checkpoint_dir: str) -> Dict[str, torch.Tensor]:
    path = find_non_lora_trainables_file(checkpoint_dir)
    if not os.path.isfile(path):
        return {}
    return _read_tensor_state(path)


def load_adapter_state(checkpoint_dir: str) -> Dict[str, torch.Tensor]:
    adapter_state = load_adapter_model_state(checkpoint_dir)
    non_lora_state = load_non_lora_trainables_state(checkpoint_dir)
    overlap = set(adapter_state).intersection(non_lora_state)
    if overlap:
        sample = ", ".join(sorted(overlap)[:5])
        raise ValueError(f"Overlapping keys between adapter_model and non_lora_trainables in {checkpoint_dir}: {sample}")
    merged = dict(adapter_state)
    merged.update(non_lora_state)
    return merged
...
def build_adapter_template(
    ...
    for key in sorted(state.keys()):
        ...
        if include_non_lora or is_lora_key(key):
            keys.append(key)
    if not keys:
        raise ValueError(f"No adapter keys matched in {checkpoint_dir}")
...
def _cache_path(
    ...
    fingerprint = "|".join(
        [
            ...
            str(os.path.getmtime(find_non_lora_trainables_file(checkpoint_dir)))
            if os.path.isfile(find_non_lora_trainables_file(checkpoint_dir))
            else "no_non_lora_trainables",
        ]
    )
...
def save_generated_adapter_checkpoint(
    ...
    if guard_root:
        guarded_makedirs(output_dir, guard_root)
    else:
    ...
    adapter_state = load_adapter_model_state(source_checkpoint_dir)
    non_lora_state = load_non_lora_trainables_state(source_checkpoint_dir)
    generated_adapter_state: Dict[str, torch.Tensor] = {
        key: value.detach().cpu() for key, value in adapter_state.items() if not is_lora_key(key)
    }
    # Non-LoRA parameters are not generated; retain the selected source values.
    source_non_lora_state = {key: value.detach().cpu() for key, value in non_lora_state.items()}
    lora_keys = {key for key in adapter_state if is_lora_key(key)}
    for key, value in generated_lora_state.items():
        if key in lora_keys:
            generated_adapter_state[key] = value
        else:
            raise KeyError(
                f"Generated key is not a LoRA adapter-model parameter: {key}. "
                f"source={source_checkpoint_dir}"
            )

    missing_adapter = sorted(lora_keys.difference(generated_adapter_state))
    if missing_adapter:
        raise KeyError(
            "Refusing to export a partially generated QwenVL LoRA adapter. "
            f"Missing adapter_model keys={missing_adapter[:8]} "
            f"source={source_checkpoint_dir}"
        )

    merged_state = {key: value.detach().cpu().contiguous() for key, value in generated_adapter_state.items()}
    if save_safetensors:
    ...
    if source_non_lora_state:
        merged_non_lora = {key: value.contiguous() for key, value in source_non_lora_state.items()}
        torch.save(merged_non_lora, os.path.join(output_dir, NON_LORA_TRAINABLES_FILENAME))
    with open(os.path.join(output_dir, "adapter_generation_meta.json"), "w", encoding="utf-8") as handle:
        json.dump(
            {
                "source_checkpoint_dir": source_checkpoint_dir,
                "parameter_policy": "generated_lora_source_non_lora",
                "source_usage": "config_dtype_shape_denormalization_and_non_lora",
                "adapter_model_keys": len(generated_adapter_state),
                "non_lora_keys": len(source_non_lora_state),
                "generated_keys": len(generated_lora_state),
            },
            handle,
            ensure_ascii=False,
            indent=2,
...
def task_output_dir(checkpoint_root: str, task: str) -> str:
    return os.path.join(checkpoint_root, f"output_{task}")
