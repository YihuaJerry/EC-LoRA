...
class MetaEBM(nn.Module):
    ...
    def pool_vector(self, v: torch.Tensor) -> torch.Tensor:
        if v.dim() == 3:
            v = v[:, 0, :]
        if v.dim() != 2:
            raise ValueError(f"Expected v to have shape [B, D], got {tuple(v.shape)}")

        if v.size(1) == self.pool_length:
            return v
    ...
    def energy_from_pooled(self, pooled: torch.Tensor, task_embed: torch.Tensor) -> torch.Tensor:
        if pooled.dim() != 2 or pooled.size(1) != self.pool_length:
            raise ValueError(
                f"Expected pooled vector shape [B, {self.pool_length}], got {tuple(pooled.shape)}"
            )
        h_v = self.vector_proj(pooled)
        h_t = self.task_proj(task_embed)
        return self.energy_head(torch.cat([h_v, h_t], dim=-1)).view(-1)

    def energy(self, v: torch.Tensor, task_embed: torch.Tensor, already_pooled: bool = False) -> torch.Tensor:
        pooled = v if already_pooled else self.pool_vector(v)
        return self.energy_from_pooled(pooled, task_embed)

    def forward(self, v: torch.Tensor, task_embed: torch.Tensor, already_pooled: bool = False) -> torch.Tensor:
        return self.energy(v, task_embed, already_pooled=already_pooled)
    ...
    def grad_v(
        ...
        energy = self.energy(x, task_embed, already_pooled=already_pooled)
    ...
    def refine(
        ...
        with torch.enable_grad():
            ...
            for _ in range(max(0, int(steps))):
                ...
                energy = float(beta) * self.energy(v, task_embed, already_pooled=False)
                ...
                if langevin_noise_std > 0.0:
                    noise_scale = float(langevin_noise_std) * math.sqrt(
                        max(2.0 * step_size / max(float(beta), 1e-12), 0.0)
                    )
