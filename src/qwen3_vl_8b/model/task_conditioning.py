...
TASK_DESCRIPTIONS: Dict[str, str] = {
    "scienceqa": "Task: ScienceQA. Input: an image and a science question. Output: the correct multiple-choice answer with visual and commonsense reasoning.",
    "vizwiz": "Task: VizWiz. Input: a user-taken image and a question from a blind user. Output: a short natural language answer.",
    "imagenet": "Task: ImageNet classification. Input: an image. Output: the object category shown in the image.",
    "vqav2": "Task: VQAv2. Input: an image and an open-ended visual question. Output: a concise answer grounded in the image.",
    "iconqa": "Task: IconQA. Input: an icon-based diagram or image and a question. Output: the correct answer option.",
    "flickr30k": "Task: Flickr30k captioning. Input: a natural image. Output: a descriptive caption for the image.",
    "grounding": "Task: visual grounding. Input: an image and a referring expression. Output: the bounding box for the referred object.",
...
def _resolve_model_path_and_dtype(
    ...
    config_dir = str(getattr(args, "eval_lora_config_dir", "") or getattr(args, "eval_qwen_config_dir", "") or "").strip()
    if not config_dir:
    ...
    model_path = str(
        model_cfg.get("pretrained_checkpoint", "")
