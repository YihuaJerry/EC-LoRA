...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Meta-EBM on LLaVA PEFT LoRA adapter checkpoints")
    ...
    parser.add_argument(
        "--task_order",
        type=str,
        default="scienceqa,vizwiz,imagenet,vqav2,iconqa,flickr30k,rec_coco,ocr_vqa",
    )
    ...
    parser.add_argument("--include_non_lora", action="store_true")
    parser.add_argument("--adapter_key_regex", type=str, default="")
    parser.add_argument("--task_condition_mode", type=str, default="dense_hidden", choices=["dense_hidden"])
    ...
    parser.add_argument("--pool_length", type=int, default=1024)
    parser.add_argument("--vector_hidden_dim", type=int, default=256)
    ...
    parser.add_argument("--trace_weight", type=float, default=1.0)
    ...
    parser.add_argument("--exp_name", type=str, default="meta_ebm_llava")
    ...
    parser.add_argument("--eval_use_flash_attn", type=str2bool, default=False)
    parser.add_argument("--eval_llava_device", type=str, default="cuda")
...
def validate_llava_tasks(tasks: Sequence[str]) -> None:
    invalid = [task for task in tasks if task not in ALLOWED_LLAVA_TASKS]
    if invalid:
        raise ValueError(
            "Meta_EBM_LLAVA received non-LLaVA tasks "
            f"{invalid}. Expected only: {', '.join(sorted(ALLOWED_LLAVA_TASKS))}. "
            "This usually means the wrong config was used; check configs/meta_ebm_llava.yaml, "
            "--task_order, and --checkpoint_root."
        )
...
def _input_score(model: MetaEBM, x: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
    ...
    energy = model.energy(x_req, cond)
...
def composition_consistency_loss(
    ...
    score_avg = 0.5 * (_input_score(model, x1, c1) + _input_score(model, x2, c2))
...
def compute_task_mean(dataset: FullAdapterDataset) -> torch.Tensor:
    ...
    if accumulator is None:
        raise ValueError("Cannot compute a task prototype from an empty dataset.")
...
def build_auto_eval_args(
    ...
    default_lora_project_dir = (
        project_root
        if os.path.basename(os.path.normpath(project_root)).lower() == "lora_llava"
        else os.path.join(project_root, "LoRA_LLAVA")
    )
    return argparse.Namespace(
        ...
        lora_project_dir=args.eval_lora_project_dir or default_lora_project_dir,
        lora_config_dir=args.eval_lora_config_dir or os.path.join(default_lora_project_dir, "config"),
        ...
        use_flash_attn=args.eval_use_flash_attn,
        llava_device=args.eval_llava_device,
...
def adapter_task_to_benchmark_task(task: str) -> str:
    return "grounding" if task == "rec_coco" else task
...
def select_after_task_benchmark_tasks(args: argparse.Namespace, tasks: Sequence[str], task_idx: int) -> Optional[List[str]]:
    ...
    for task in list(tasks[: task_idx + 1]) + list(UNSEEN_BENCHMARK_TASKS):
        benchmark_task = adapter_task_to_benchmark_task(task)
        if benchmark_task not in ordered:
            ordered.append(benchmark_task)
    return ordered
...
def train(args: argparse.Namespace) -> str:
    ...
    checkpoint_root = os.path.abspath(
        os.path.expanduser(
            ...
            or os.path.join(project_root, "CMA_ES", "LLAVA", "checkpoints")
        )
    )
    ...
    if resume_payload is not None:
        if not resume_payload.get("task_prototypes") or not resume_payload.get("task_embeddings"):
            raise ValueError("The resume checkpoint predates full-vector prototypes and dense task conditions.")
        task_embeddings = {task: value.detach().float().cpu() for task, value in resume_payload["task_embeddings"].items()}
        task_condition_metadata = dict(resume_payload.get("task_condition_metadata", {}))
        if task_condition_metadata.get("mode") != "dense_hidden":
            raise ValueError("Cannot resume a non-dense LLaVA task-condition checkpoint.")
    else:
        task_embeddings, task_condition_metadata = build_task_condition_embeddings(args, tasks)
    ...
    if resume_payload is not None:
        ...
        if start_task_idx and not task_prototypes:
            raise ValueError("The resume checkpoint lacks full task_prototypes; retrain with the full-vector method.")
    ...
    if args.auto_eval_after_train and args.eval_after_each_task:
        ...
        print("[auto-eval] training finished, running Meta-EBM refinement + LLaVA generation evaluation")
        model.to("cpu")
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        from eval_meta_ebm import run_evaluation
