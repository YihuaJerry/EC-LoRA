...
def _load_safetensors(path: str) -> Dict[str, torch.Tensor]:
    try:
        ...
        raise ModuleNotFoundError(
            ...
            "Install it in the PAMI conda env or use checkpoints with adapter_model.bin."
        ) from exc
...
def build_adapter_template(
    ...
    for key in keys:
        ...
        mean, std = tensor_stats(state[key])
        stats[key] = {"mean": float(mean.item()), "std": float(std.item())}
...
def reconstruct_lora_state(
    ...
    for key in template["keys"]:
        ...
        if normalize_mode == "per_tensor":
            # Vectors are normalized per tensor; restore them in the coordinate
            # system of the source adapter that will provide the classifier head.
...
def task_output_dir(checkpoint_root: str, task: str, output_suffix: str = "qwen3_8b") -> str:
    suffix = str(output_suffix or "").strip()
    name = f"output_{task}_{suffix}" if suffix else f"output_{task}"
    return os.path.join(checkpoint_root, name)


def _task_output_markers(task: str, output_suffix: str = "qwen3_8b") -> List[str]:
    suffix = str(output_suffix or "").strip()
    markers = []
    if suffix:
        markers.append(f"output_{task}_{suffix}")
    markers.append(f"output_{task}")
    return markers
...
def resolve_checkpoint_path(
    ...
    checkpoint_root: str,
    task: str,
    output_suffix: str = "qwen3_8b",
) -> str:
    candidates: List[str] = []
    if reference_path:
        name = os.path.basename(os.path.normpath(reference_path))
        if name.startswith("checkpoint-"):
            candidates.append(os.path.join(task_output_dir(checkpoint_root, task, output_suffix), name))

        normalized = os.path.normpath(reference_path)
        parts = normalized.split(os.sep)
        for marker in _task_output_markers(task, output_suffix):
            if marker in parts:
                idx = parts.index(marker)
                suffix = os.path.join(*parts[idx + 1 :]) if idx + 1 < len(parts) else ""
                candidates.append(os.path.join(task_output_dir(checkpoint_root, task, output_suffix), suffix))
...
def discover_task_checkpoints(
    ...
    for task in tasks:
        out_dir = task_output_dir(checkpoint_root, task, output_suffix)
...
def parse_task_order(task_order: str) -> List[str]:
    tasks = [token.strip().lower() for token in str(task_order).split(",") if token.strip()]
    if not tasks:
