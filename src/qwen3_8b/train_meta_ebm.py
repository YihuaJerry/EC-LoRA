...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Meta-EBM on Qwen PEFT LoRA adapter checkpoints")
    ...
    parser.add_argument("--exp_name", type=str, default="meta_ebm_qwen")
...
def build_auto_eval_args(
    ...
    return argparse.Namespace(
        ...
        checkpoint_output_suffix=args.checkpoint_output_suffix,
        lora_project_dir=args.eval_lora_project_dir or os.path.join(project_root, "models", "LoRA_Qwen"),
        lora_config_dir=args.eval_lora_config_dir or os.path.join(project_root, "models", "config", "LoRA_Qwen"),
...
def train(args: argparse.Namespace) -> str:
    ...
    checkpoint_root = os.path.abspath(
        os.path.expanduser(
            ...
            or os.path.join(project_root, "CMA_ES", "Qwen", "checkpoints")
        )
    )
    ...
    task_to_specs = discover_task_checkpoints(
        ...
        output_suffix=args.checkpoint_output_suffix,
    )
    ...
    if resume_payload is not None and resume_payload.get("task_embeddings"):
        ...
        task_embeddings, task_condition_metadata = build_task_condition_embeddings(
            ...
            default_lora_config_name="LoRA_Qwen",
        )
    ...
    result_guard = OutputDirGuard(run_dir, "training result directory")
